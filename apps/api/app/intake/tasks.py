...
Order of preference:
  ...
  2. Gemini writes tasks from the site files (app.optimizer.task_gen) when a key is configured
...
from app.schemas import Task, TaskKind
...
GENERIC_TASKS: list[dict] = [
    {
        "id": "g01_pricing",
        "prompt": "Find the pricing page of this site and stop there.",
        "expected_url_pattern": r"pric|plans?",
        "tags": ["navigation", "pricing"],
    },
    {
        "id": "g02_docs",
        "prompt": "Find the documentation (docs or guides) section of this site and stop on it.",
        "expected_url_pattern": r"docs?|documentation|guides?",
        "tags": ["navigation", "docs"],
    },
    {
        "id": "g03_contact",
        "prompt": "Find the page where you can contact this company or get support, and stop there.",
        "expected_url_pattern": r"contact|support|help",
        "tags": ["navigation", "contact"],
    },
    {
        "id": "g04_signup",
        "prompt": "Find the page where a new user signs up, registers or gets started, and stop there.",
        "expected_url_pattern": r"sign-?up|register|get-?started|start|waitlist|trial",
        "tags": ["navigation", "signup"],
    },
    {
        "id": "g05_about",
        "prompt": "Find the page that describes the company or the team behind this site, and stop there.",
        "expected_url_pattern": r"about|company|team",
        "tags": ["navigation", "about"],
    },
    {
        "id": "g06_blog",
        "prompt": "Find the blog, changelog or news section of this site and stop on it.",
        "expected_url_pattern": r"blog|changelog|news|releases?",
...
def pick_tasks(
    site_id: str, version: str, *, source_url: str | None = None, n_generated: int = 8, persist: bool = True
) -> tuple[list[Task], str]:
    ...
    if not tasks and settings.gemini_api_key:
        try:
            from app.optimizer.task_gen import generate_tasks
        ...
        if heading and heading.get_text(" ", strip=True):
            tasks.append(
                Task(
                    ...
                    expected_answer=heading.get_text(" ", strip=True),
                    max_steps=3,
                )
            )
        seen = set()
        ...
        if not tasks:
            ...
            tasks = [
                Task(
                    id="quick_home",
                    kind="url",
                    prompt="Open the homepage and stop there.",
                    expected_url_pattern=r"^(?:index\.html)?(?:[?#].*)?$",
                    max_steps=3,
                )
            ]
    return [
        t.model_copy(update={"max_steps": min(t.max_steps, 6), "timeout_s": 60.0})
