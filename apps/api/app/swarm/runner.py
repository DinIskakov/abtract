...
from app.metrics.score import aggregate, judge
from app.modal_app import app, base_image, browser_image, secrets, volumes
from app.models.registry import get_model
from app.schemas import AgentKind, Episode, RunSummary, Task

OnEpisode = Callable[[Episode], None]
OnStart = Callable[[list[dict[str, Any]]], None]
log = logging.getLogger("app.swarm")
...
class BudgetExceeded(RuntimeError):
    ...
    def __init__(self, estimate: dict[str, Any], budget_usd: float):
        ...
        super().__init__(
            f"estimated cost ${estimate['total_usd']:.2f} for {estimate['episodes']} episodes exceeds "
...
def run_episode_sync(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        import app.agents as agents  # looked up at call time so tests can monkeypatch app.agents.run_agent
...
def _fan_out(
    payloads: list[dict[str, Any]],
    concurrency: int,
    on_episode: OnEpisode | None,
    *,
    execute: Callable[[dict[str, Any]], dict],
    ...
    if pending:  # only possible when the budget ran out
        log.warning(
            "budget exhausted after $%.4f (budget $%.2f): %d of %d episodes not started",
            spent,
            budget_usd or 0.0,
            len(pending),
            len(payloads),
        ...
        for p in pending:
            ep = error_episode(
                p, f"budget exhausted: ${spent:.4f} spent of ${budget_usd:.2f} before this episode started"
            )
            store.save_episode(ep)
            episodes.append(ep)
            if on_episode:
...
def _fan_out_local(
    ...
    return _fan_out(
        payloads, concurrency, on_episode, execute=run_episode_sync, budget_usd=budget_usd, on_start=on_start
    )
...
def _fan_out_modal(
    ...
    return _fan_out(
        payloads, concurrency, on_episode, execute=run_episode.remote, budget_usd=budget_usd, on_start=on_start
    )
...
def run_swarm(
    ...
    *,
    ...
    log.info(
        "swarm estimate: %d episodes, ~$%.2f (%s)",
        estimate["episodes"],
        estimate["total_usd"],
    ...
    if local:
        ...
        episodes = _fan_out_modal(
            payloads, on_episode, budget_usd=budget_usd, concurrency=concurrency, on_start=on_start
        )

    spent = sum(e.cost_usd for e in episodes)
    if budget_usd is not None and spent > budget_usd:
        log.warning(
            "swarm spent $%.4f, over the $%.2f budget%s", spent, budget_usd, " (in-flight calls were allowed to finish)"
        )
    else:
...
def serve_site_locally(port: int | None = None) -> tuple[str, Callable[[], None]]:
    ...
    Prefers app.hosting.serve.create_app() under uvicorn (records SiteEvents, needed for action tasks);
    ...
    try:
        ...
        from app.hosting.serve import create_app
