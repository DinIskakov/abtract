...
from app.models.registry import select_models
from app.schemas import Episode, RunSummary, SiteVersion, Task
from app.swarm.report import estimate_table, print_run, versions_table
from app.swarm.runner import BudgetExceeded, estimate_run_cost, run_swarm, serve_site_locally
...
def show_estimate(
    ...
    *,
    ...
    if budget_usd is not None and estimate["total_usd"] > budget_usd:
        raise SystemExit(
            f"swarm: estimated ${estimate['total_usd']:.2f} exceeds --budget-usd {budget_usd:.2f}; "
...
def ensure_site_in_store(site_id: str, version: str) -> None:
    ...
    for src in candidates:
        if src.is_dir() and any(src.glob("*.html")):
            ...
            return
    raise SystemExit(
        f"site version {site_id}/{version} not found at {store.site_dir(site_id, version)} "
        f"(import it with store.import_site or scripts/import_demo_site.py)"
    )


def setup_site(
    site_id: str, version: str, *, local: bool, site_base_url: str | None = None
    ...
    if not settings.site_base_url:
        raise SystemExit(
            "cloud runs need the deployed site server URL: set ABTRACT_SITE_BASE_URL in .env or pass "
...
def load_tasks(spec: str, site_id: str, version: str, *, limit: int | None = None) -> list[Task]:
    ...
    if spec.strip().lower() == "generate":
        from app.optimizer.task_gen import generate_tasks, save_tasks
...
def _progress(total: int) -> Callable[[Episode], None]:
    ...
    def on_episode(ep: Episode) -> None:
        ...
        console.print(
            f"[{done['n']:3d}/{total}] {status} {ep.model_id:22s} {ep.agent_kind.value:6s} {ep.task_id:24s} "
            f"steps={ep.n_steps:2d} {ep.duration_s:6.1f}s ${ep.cost_usd:.4f}{extra}",
            highlight=False,
        )
...
def swarm_main(
    *,
    ...
    try:
        ...
        console.print(
            f"swarm: {site}/{version} at {site_url}  models={model_ids} agents={agent_kinds} "
        ...
        try:
            run = run_swarm(
                ...
                run_id=run_id,
                on_episode=_progress(estimate["episodes"]),
        ...
        console.print(
            f"swarm: done in {time.time() - t0:.0f}s, spent ${run.overall.total_cost_usd:.4f}; "
    ...
    try:
        ...
        for i in range(iterations + 1):
            ...
            if local:
                ...
                from app.optimizer.rewrite import optimize_site_remote
