...
from app.schemas import Episode, MetricBlock, RunSummary, Task, TaskKind
...
_NOISE_WORDS = {"per", "usd", "dollar", "dollars", "the", "a", "an", "is", "it", "its", "it's"}
_UNITS = {
    "hr": "hour",
    "hrs": "hour",
    "hours": "hour",
...
def _llm_answer_judge(task: Task, answer: str) -> tuple[bool, str] | None:
    ...
    if not settings.gemini_api_key:
        return None
    try:
        from app.models.llm import ChatMessage, chat, extract_json
...
def judge(episode: Episode, task: Task, *, use_llm_judge: bool = True) -> Episode:
    ...
    if ev is not None:
        ...
        want = f"{task.expected_event!r}" + (
            f" matching {task.expected_event_match}" if task.expected_event_match else ""
        )
