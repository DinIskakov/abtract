...
Local dev:  uv run python -m app.hosting.serve --port 8000
...
from app.schemas import SiteEvent
...
def create_app() -> FastAPI:
    ...
    def index(request: Request):
        sites = [
            {"site_id": s, "versions": [v.model_dump() for v in store.list_versions(s)]} for s in store.list_sites()
        ]
        ...
        for s in sites:
            vers = s["versions"] or [
                {"version": p.name} for p in (store.root() / "sites" / s["site_id"]).iterdir() if p.is_dir()
            ]
            links = " ".join(
