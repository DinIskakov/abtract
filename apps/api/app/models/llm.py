...
from app.schemas import ModelSpec, Usage
        ...
        try:
            ...
            if status in (401, 403):
                if spec.provider == "gemini":
                    raise LLMError(
                        f"Google rejected the Gemini credential (HTTP {status}). Check the key and project permissions "
                        "in Google AI Studio, update GEMINI_API_KEY in abtract-secrets, and redeploy before retrying Optimize."
                    ) from e
                raise LLMError(
                    f"{spec.id}: provider rejected the credential (HTTP {status}); check its endpoint token and permissions"
                ) from e
            last_err = e
...
def _modal_model_names(base_url: str) -> list[str]:
    with _resolve_lock:
        if base_url not in _MODAL_MODEL_NAMES:
            from app.models import registry  # looked up at call time so tests can monkeypatch discover_modal_models
            ...
            try:
                ...
                log.warning(
                    "could not list Modal models (%s: %s); using configured model names unchanged", type(e).__name__, e
                )
...
def resolve_modal_model(spec: ModelSpec) -> str:
    ...
    if resolved != spec.model:
        ...
        log.warning(
            "modal model %s: '%s' is not among the %d names in /v1/models; sending it unchanged "
            "(set ABTRACT_MODEL_%s or run scripts/list_models.py)",
            spec.id,
            spec.model,
            len(names),
...
def _chat_openai_compatible(spec, messages, json_mode, max_out, temperature, timeout_s=None) -> LLMResponse:
    ...
    client = OpenAI(
        api_key=settings.modal_proxy_token,
        base_url=modal_base_url(spec),
        timeout=timeout_s if timeout_s is not None else settings.step_timeout_s,
        max_retries=0,
    )
    oa_messages = []
    for m in messages:
        if isinstance(m.content, str):
            ...
            for p in m.content:
                if p["type"] == "text":
                    ...
                    parts.append(
                        {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{p['png_base64']}"}}
                    )
            oa_messages.append({"role": m.role, "content": parts})
    kwargs: dict[str, Any] = dict(
        model=resolve_modal_model(spec), messages=oa_messages, max_tokens=max_out, temperature=temperature
    )
    if json_mode:
    ...
    options = (
        {
            "http_options": types.HttpOptions(
                timeout=max(1, int(timeout_s * 1000)), retry_options=types.HttpRetryOptions(attempts=1)
            )
        }
...
def _chat_mock(spec, messages, json_mode) -> LLMResponse:
    ...
    if reply is None:
        reply = (
            json.dumps({"thought": "mock model", "action": {"type": "give_up", "reason": "mock"}})
            if json_mode
            else "mock reply"
        )
