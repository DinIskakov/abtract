...
from app.models.pricing import SHARED_ENDPOINT_MODELS
...
def _modal_models(*, vision_only: bool = False) -> list[ModelSpec]:
    return sorted(
        (m for m in MODELS.values() if m.provider == "modal" and (m.supports_vision or not vision_only)), key=cost_rank
    )
...
def select_models(spec: str) -> list[ModelSpec]:
    ...
    for token in tokens:
        ...
        if key == "all":
            ...
            raise ValueError(
                f"unknown model {token!r}; known ids: {', '.join(MODELS)}; "
