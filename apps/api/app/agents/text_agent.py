...
class TextAgent(BaseAgent):
    ...
    def _load(self, method: str, url: str, data: list[tuple[str, str]] | None = None) -> None:
        ...
        for k, v in data or []:
            if k in form:  # repeated names (checkbox groups) -> list
                ...
                form[k] = v
        try:
            r = self.client.post(url, data=form) if method == "POST" else self.client.get(url, params=form or None)
    ...
    def _describe(self, el: TextElement) -> str:
        ...
        if el.kind == "select":
            ...
            return (
                f"[{el.id}] select name={_attr(tag, 'name') or '-'}{lab} selected={sel!r} ({len(el.options)} options)"
            )
        if el.kind in ("checkbox", "radio"):
            chk = self.checked.get(el.id, tag.has_attr("checked"))
            lab = f' label="{el.label}"' if el.label else ""
    ...
    def _click(self, el: TextElement) -> None:
        ...
        if el.kind == "button":
            raise ActionError(
                f"[{el.id}] is a type=button control that only works with JavaScript; nothing happened. Look for a submit button or a link instead"
            )
        # submit
        form = self._form_of(tag)
        if form is None:
