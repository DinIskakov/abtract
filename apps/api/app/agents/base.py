...
from app.schemas import Action, AgentKind, Episode, ModelSpec, Step, Task
...
def parse_reply(text: str) -> tuple[str | None, Action | None, str | None]:
    ...
    if canon is None:
        return (
            thought,
            None,
            f"unknown action type {atype!r}; use navigate/click/type/select/scroll/back/answer/give_up",
        )
...
class BaseAgent:
    ...
    def check_origin(self, url: str) -> str:
        ...
        if not same_origin(full, self.site_url):
            raise ActionError(
                f"refused: {full} is not on this site's origin ({urlparse(self.site_url).netloc}); stay on the site"
            )
        return full
    ...
    # ---- prompt construction
    def build_messages(
        self, task: Task, history: list[dict[str, Any]], obs: Observation, step_index: int, max_steps: int
    ) -> list[ChatMessage]:
        hist_lines = []
        for h in history:
        ...
        try:
            ...
            for i in range(max_steps):
                ...
                step = Step(
                    index=i,
                    url=url_before,
                    observation_chars=len(obs.text),
                    thought=thought,
                    action=action or Action(type="noop"),
                ...
                history.append(
                    {
                        "index": i,
                        "thought": thought,
                        "action": action.model_dump(exclude_none=True) if action else None,
                        "error": step.error,
                    }
                )
                if step.error:
