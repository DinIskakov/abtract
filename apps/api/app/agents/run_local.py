"""Run one agent against one site locally and print the Episode.
    ...
    uv run python -m app.agents.run_local --agent text --model mock --site-url http://localhost:9000/ \
...
from app.schemas import AgentKind, Task, TaskKind
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--screenshot-dir",
        help="vision agent: where to write NN.png (default: <data_dir>/runs/<run_id>/screenshots/<episode>/)",
    )
    ...
    from app.agents.base import BaseAgent  # noqa: F401  (import check)
    ...
    if kind == AgentKind.text:
        from app.agents.text_agent import TextAgent as Impl
    elif kind == AgentKind.dom:
        from app.agents.dom_agent import DomAgent as Impl
    else:
        from app.agents.vision_agent import VisionAgent as Impl
    agent = Impl(
        model_id=args.model,
        run_id=args.run_id,
