...
class Settings(BaseSettings):
    ...
    dashboard_password: str = Field("", validation_alias="ABTRACT_DASHBOARD_PASSWORD")
    job_swarm_budget_usd: float = Field(5, validation_alias="ABTRACT_JOB_SWARM_BUDGET_USD")
    job_swarm_concurrency: int = Field(16, validation_alias="ABTRACT_JOB_SWARM_CONCURRENCY")
    data_dir: Path = Field(
    ...
    episode_timeout_s: int = Field(600, validation_alias="ABTRACT_EPISODE_TIMEOUT_S")
...
settings = Settings()  # type: ignore[call-arg]  # pydantic-settings resolves validation aliases from the environment
