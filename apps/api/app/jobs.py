...
optimize_site, write_findings) so tests can monkeypatch `app.jobs.<name>`.
"""
...
from app.hosting import urls
from app.intake.mirror import mirror_to_store
from app.intake.tasks import pick_quick_tasks, pick_tasks
from app.modal_app import APP_NAME, app, base_image, secrets, volumes
from app.models.registry import DEFAULT_SWARM, get_model
from app.optimizer.findings import rule_based_findings, write_findings
from app.optimizer.preview import attempt, inspect_page, summarize_page, update_attempts
from app.optimizer.rewrite import optimize_site
from app.schemas import ActiveAttempt, AgentKind, Episode, Job, JobPreview, PreviewAttempt, RunSummary, Task
from app.swarm.runner import run_swarm, serve_site_locally
...
def _progress(
    job: Job, phase: str | None = None, done: int | None = None, total: int | None = None, log_line: str | None = None
) -> None:
...
def _run_swarm_phase(job: Job, site_id: str, version: str, tasks: list[Task], *, phase: str, local: bool) -> RunSummary:
    ...
    job.live_preview = JobPreview(
        ...
        total=total,
        task_sample=[t.prompt for t in tasks[:4]],
    ...
    def on_start(payloads: list[dict[str, Any]]) -> None:
        for payload in payloads:
            task = payload["task"]
            job.live_preview.active.append(
                ActiveAttempt(
                    task_id=task["id"],
                    prompt=task["prompt"],
    ...
    def on_episode(ep: Episode) -> None:
        ...
        job.live_preview.active = [
            a
            for a in job.live_preview.active
    ...
    _progress(
        ...
        log_line=f"run {run.run_id}: {100 * o.success_rate:.0f}% success over {o.episodes} "
...
def _run_intake(job: Job, *, local: bool) -> None:
    ...
    msg = (
        f"mirrored {len(report.pages)} pages and {len(report.assets)} assets into {sv.site_id}/{sv.version}"
    ...
    _progress(
        job,
        log_line=f"{len(tasks)} tasks ({how}): "
        + ", ".join(t.id for t in tasks[:8])
        + (" ..." if len(tasks) > 8 else ""),
    )
...
def _run_loop(job: Job, *, local: bool) -> None:
    ...
    if baseline is None:
        ...
        baseline = _run_swarm_phase(
            job, site_id, version, tasks, phase=f"Running swarm on {version} (baseline)", local=local
        )
    else:
        job.site_version = version
        _progress(
            job,
            log_line=f"baseline: run {baseline.run_id} on {version} "
