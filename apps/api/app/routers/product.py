"""Product API over the shared store.

Every handler reloads the Modal Volume first; product traffic is low enough for that.
    ...
    GET  /api/demo-url                                  {"url": <hosted demo site>} (app.hosting.urls, with fallback)
...
`app.jobs` (job runner) and `app.hosting.urls` are imported lazily inside the handlers so the dashboard
still serves everything else if they are missing or broken.

Local development is served by ``app.main``. Modal registers the same API as ``dashboard``.
"""
...
from app.schemas import AgentKind, Episode, Job, JobPreview, MetricBlock, RunSummary, SiteVersion
...
def _compare_group(ga: dict[str, MetricBlock], gb: dict[str, MetricBlock]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "key": k,
            "a": ga[k].model_dump() if k in ga else None,
            "b": gb[k].model_dump() if k in gb else None,
            "delta": _delta(ga.get(k), gb.get(k)),
        }
        for k in keys
    ]
...
def _build_job(req: JobRequest) -> Job:
    ...
    if req.model_ids is None:
        model_ids = (
            list(run.model_ids) if run and run.model_ids else list(defaults) or (["mock"] if "mock" in models else [])
        )
    ...
    common: dict[str, Any] = {
        "model_ids": model_ids,
        "agent_kinds": [AgentKind(k) for k in kinds],
        "budget_usd": allowance,
        "scan_mode": run.scan_mode if run else req.scan_mode,
    }
    if req.type == "intake":
    ...
    return Job(
        type="loop",
        url=_normalize_url(req.url) if req.url else run.site_url or None,
        site_id=req.site_id,
...
def create_app() -> FastAPI:
    ...
    if not modal.is_local() and not settings.dashboard_password:
        raise RuntimeError("Set ABTRACT_DASHBOARD_PASSWORD in abtract-secrets before hosting the product API")
    app = FastAPI(title=settings.app_name, version=settings.app_version, docs_url="/api/docs", redoc_url=None)
    ...
    if settings.dashboard_password:
        ...
        async def authenticate(request: Request, call_next: RequestResponseEndpoint) -> Response:
            ...
            try:
                ...
                valid = (
                    scheme.lower() == "basic"
                    and hmac.compare_digest(user.encode(), b"abtract")
                    and hmac.compare_digest(supplied.encode(), settings.dashboard_password.encode())
                )
            ...
            if not valid:
                return Response(
                    status_code=401,
                    headers={"WWW-Authenticate": 'Basic realm="abtract", charset="UTF-8"'},
                )
            return await call_next(request)
                    ...
                    "versions": [
                        {
                            **v.model_dump(),
                            "runs": [
                                _run_brief(r) for r in runs if r.site_id == site_id and r.site_version == v.version
                            ],
                        }
                        for v in versions
                    ],
    ...
    def demo_url() -> dict[str, Any]:
        ...
        try:
            from app.hosting.urls import (
                demo_site_url as _demo_site_url,  # resolved by module path, so tests can patch it
            )
    ...
    def first_look(job_id: str) -> JobPreview | None:
        ...
        from app.optimizer.preview import fetch_first_look
    ...
    def site_file(site_id: str, version: str, path: str) -> PlainTextResponse:
    ...
    def screenshot(run_id: str, ep_id: str, name: str) -> FileResponse:
...
try:
    ...
    from app.modal_app import app as _modal_app
    from app.modal_app import base_image, secrets, volumes

    @_modal_app.function(image=base_image, volumes=volumes, secrets=secrets)  # type: ignore[arg-type]
    @modal.concurrent(max_inputs=50)
    @modal.asgi_app(label="dashboard")
    def dashboard() -> FastAPI:
        return create_app()
