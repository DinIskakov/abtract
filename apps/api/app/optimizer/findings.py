...
from app.metrics.score import compare, metric_block
from app.models.llm import ChatMessage, chat  # noqa: F401  (chat is patchable in tests)
from app.models.registry import get_model
from app.optimizer.rewrite import OPTIMIZER_MODEL, failure_section, metrics_tables
from app.schemas import Episode, RunSummary, Task, TaskKind

MODE_LABEL = {
    "wrong_answer": "wrong answer",
    "wrong_page": "ended on the wrong page",
    "no_event": "form never submitted",
    "max_steps": "ran out of steps",
    "gave_up": "gave up",
    "error": "crashed",
    "timeout": "timed out",
}
...
def _headline(run: RunSummary, episodes: list[Episode], tasks: list[Task]) -> list[str]:
    ...
    return [
        f"# Findings for {run.site_id}/{run.site_version}",
        "",
...
def _worst_tasks(episodes: list[Episode], tasks: list[Task], n: int = 5) -> list[str]:
    ...
    for _, _, tid, eps, failed in rows[:n]:
        ...
        out.append(
            f"- **{tid}** ({kind}) failed {len(failed)}/{len(eps)}; mostly {MODE_LABEL.get(mode, mode)} ({cnt}). "
...
def _suggestions(episodes: list[Episode], tasks: list[Task]) -> list[str]:
    ...
    if js_only:
        out.append(
            f"- Text agents fail where browser agents succeed on {tl(js_only)}: that content only exists after "
            "JavaScript runs. Put the same facts and links in the server-rendered HTML."
        )
    if visual_only:
        out.append(
            f"- The DOM agent fails where the vision agent succeeds on {tl(visual_only)}: the information is "
            "visual-only (canvas, image, CSS-drawn). Add it as real text or a table as well."
        )
    if vision_only_fail:
        out.append(
            f"- The vision agent fails where the DOM agent succeeds on {tl(vision_only_fail)}: something in the "
    ...
    if action_eps and _rate(action_eps) == 0.0:
        out.append(
            "- Every form/button task failed: the forms are hard to operate. Label every input with <label for>, "
            "use a real <button type=submit>, and make the submit endpoint a plain form action."
        )
    if modes.get("max_steps", 0) / n_fail >= 0.3:
        out.append(
            f"- {modes['max_steps']} attempts ran out of steps: navigation is unclear. Use a plain <nav> with "
            "descriptive link text so agents find the right page quickly."
        )
    if modes.get("wrong_answer", 0) / n_fail >= 0.3:
        out.append(
            f"- {modes['wrong_answer']} attempts reported a wrong answer: facts are ambiguous, stale or stated "
            "differently in different places (hidden text, tabs, footnotes). State each fact once, plainly."
        )
    if modes.get("gave_up", 0) / n_fail >= 0.3:
        out.append(
            f"- {modes['gave_up']} attempts gave up: agents could not find the content at all. Check that the "
            "answer is visible as text on a page reachable from the home page."
        )
    if (modes.get("error", 0) + modes.get("timeout", 0)) / n_fail >= 0.3:
        out.append(
            f"- {modes.get('error', 0) + modes.get('timeout', 0)} attempts crashed or timed out: the site or the "
            "agents were unreliable (slow pages, blocked requests). Re-run before drawing conclusions."
        )
    if not failed:
        out.append(
            "- Nothing tripped the agents: every attempt succeeded. Optimize for fewer steps and lower cost next."
        )
    if len(out) == 2:
        out.append(
            "- Failures are spread evenly across agent kinds and modes; look at the per-task traces in the "
...
def _baseline_line(baseline: RunSummary, run: RunSummary) -> list[str]:
    ...
    return [
        "",
        f"**Compared with {baseline.site_version}** (run {baseline.run_id}): success {_pct(sr['a'])} -> "
...
def rule_based_findings(
    run: RunSummary, episodes: list[Episode], tasks: list[Task], baseline: RunSummary | None = None
) -> str:
...
def llm_findings(
    run: RunSummary, episodes: list[Episode], *, model_id: str | None = None, char_cap: int = 40_000
) -> str | None:
    ...
    msgs = [
        ChatMessage(role="system", content=LLM_SYSTEM),
