...
from app.models.llm import ChatMessage, chat, extract_json  # noqa: F401  (patchable)
from app.models.registry import get_model
from app.optimizer.rewrite import site_files
from app.schemas import Task, TaskKind

GENERATOR_MODEL = "gemini-flash"
_API_RE = re.compile(
    r"""(?:action\s*=\s*|fetch\(\s*|url\s*[:=]\s*|open\(\s*['"][A-Z]+['"]\s*,\s*)['"]([^'"]*api/[A-Za-z0-9_\-/]+)['"]"""
)
...
def generate_tasks(
    site_id: str, version: str, n: int = 8, *, model_id: str = GENERATOR_MODEL, site_dir: Path | None = None
) -> list[Task]:
    ...
    user = (
        f"Write {n} tasks ({max(1, n // 2)} answer, {max(1, n // 4)} url, {max(1, n - n // 2 - n // 4)} action) "
    ...
    r = chat(
        get_model(model_id),
        [ChatMessage(role="system", content=RULES), ChatMessage(role="user", content=user)],
        json_mode=True,
