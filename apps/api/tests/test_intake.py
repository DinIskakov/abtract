...
from app.intake.mirror import MirrorReport, local_path_for, mirror_site, mirror_to_store, site_id_for_url
from app.intake.tasks import pick_tasks
...
def cases_server():
    ...
    _CasesHandler.routes = {
        "/app/": (
            ...
            f"""<!doctype html><html><head><base href="{origin}/app/">
        ...
        "/app/docs/": (
            "text/html",
            f"<html><body><a href='{origin}/app/pricing#plans'>Plans</a></body></html>".encode(),
        ),
        ...
        "/app/css/site.css": (
            "text/css",
            f"body{{background:url({origin}/app/img/bg.png)}} @import '{origin}/app/css/x.css';".encode(),
        ),
...
def test_local_path_for_cases():
    ...
    assert (
        local_path_for("https://x.modal.run/other/a.png", root, kind="asset", content_type="image/png")
        == "__/other/a.png"
    )
...
def test_pick_tasks_gemini_failure_falls_through(tmp_store, monkeypatch):
    import app.optimizer.task_gen as tg
