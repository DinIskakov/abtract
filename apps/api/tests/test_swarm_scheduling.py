...
from app.schemas import RunSummary, Task
...
def test_slow_attempt_does_not_block_new_attempts(tmp_path, monkeypatch, local):
    ...
    payloads = runner.build_payloads(
        RunSummary(site_id="demo", site_version="v0", site_url="u"), tasks, ["mock"], ["text"]
    )
...
def test_short_checks_start_first_without_changing_coverage():
    tasks = [
        Task(id="long", kind="action", prompt="Sign up", max_steps=20),
