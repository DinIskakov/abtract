...
from app.intake.mirror import MirrorReport
from app.metrics.score import aggregate
from app.optimizer import findings
from app.schemas import Action, AgentKind, Episode, Job, RunSummary, SiteVersion, Step, Task
...
def _fake_episode(
    run: RunSummary, task: Task, model_id: str, kind: str, *, success: bool, mode: str | None = None, steps: int = 2
) -> Episode:
    ep = Episode(
        run_id=run.run_id,
        site_id=run.site_id,
        ...
        model_id=model_id,
        success=success,
        failure_mode=None if success else mode,
        judge_reason="ok" if success else "expected x, got y",
        finished_at=time.time() + 1,
    )
    ep.steps = [
        Step(index=i, url=run.site_url, action=Action(type="navigate", url="pricing.html")) for i in range(steps)
    ]
...
def make_fake_run_swarm(calls: list, *, text_success: bool = False):
    ...
    def fake_run_swarm(site_id, site_version, tasks, model_ids, agent_kinds, *, site_url, local, on_episode=None, **kw):
        ...
        run = RunSummary(
            ...
            agent_kinds=[AgentKind(k) for k in agent_kinds],
        )
        store.save_run(run)
        episodes = []
        for m in model_ids:
...
def _fake_mirror_to_store(url, *, site_id=None, **kw):
    ...
    sv = SiteVersion(
        site_id=site_id, version="v0", notes=f"Mirrored from {url}", changed_files=["index.html", "pricing.html"]
    )
...
def _fake_optimize_site(site_id, version, run_id, *, local):
    ...
    (store.site_dir(site_id, new) / "pricing.html").write_text(
        "<html><body><table><tr><td>$3.95</td></tr></table></body></html>"
    )
    sv = SiteVersion(
        site_id=site_id,
        version=new,
        parent=version,
        notes=f"fake optimizer from {run_id}",
...
def test_loop_job_optimizes_and_reruns(faked, monkeypatch):
    ...
    baseline = jobs.run_swarm(
        "fake-site", "v0", TASKS, ["mock"], ["text"], site_url="http://x/s/fake-site/v0/", local=True
    )
    faked["swarm"].clear()
    ...
    _phase_order(
        saved, "Optimizing v0", "Running swarm on v1", "Optimizing v1", "Running swarm on v2", "Writing findings"
    )
...
def test_write_findings_rule_based(tmp_store, monkeypatch):
    ...
    better = make_fake_run_swarm([], text_success=True)(
        "s", "v1", TASKS, ["mock"], ["text", "dom", "vision"], site_url="http://x/s/s/v1/", local=True
    )
