...
from app.hosting.serve import create_app as site_app
from app.routers.product import JobRequest, _build_job, create_app
from app.schemas import Episode, Job, RunSummary, SiteVersion, Task
...
def test_product_password_protects_job_api(data, monkeypatch):
    ...
    with TestClient(create_app()) as client:
        ...
        assert client.get("/api/jobs", auth=("abtract", "wrong")).status_code == 401
        assert client.get("/api/jobs", auth=("abtract", "test-password")).status_code == 200
...
def test_loop_persists_exact_baseline(data):
    run = RunSummary(
        site_id="demo",
        site_version="v0",
        site_url="http://example/s/demo/v0/",
        model_ids=["mock"],
        agent_kinds=["text"],
    )
    store.save_run(run)
    job = _build_job(
        JobRequest(type="loop", site_id="demo", site_version="v0", run_id=run.run_id, url="https://original.example/")
    )
    store.save_job(job)
...
def test_cloud_budget_stops_new_submissions(data, monkeypatch):
    ...
    eps = runner._fan_out_modal(
        runner.build_payloads(run, tasks, ["mock"], ["text"]), None, budget_usd=1, concurrency=2
    )
...
def test_vision_swarm_screenshots_reach_dashboard(data, monkeypatch):
    ...
    try:
        ...
        ep = Episode(
            **runner.run_episode_sync(runner.build_payloads(run, [task], ["mock"], ["vision"], use_llm_judge=False)[0])
        )
        assert ep.success
        with TestClient(create_app()) as client:
