...
from app.routers.product import VolumeAccessMiddleware


def test_volume_requests_are_exclusive_but_health_checks_are_not_blocked():
    async def scenario():
        ...
        second = asyncio.create_task(call("/api/jobs/second"))
        screenshot = asyncio.create_task(call("/screenshots/r/e/1.png"))
        static = asyncio.create_task(call("/healthz"))
        await asyncio.wait_for(static_done.wait(), timeout=1)
        assert entered == ["/api/jobs/first", "/healthz"]
        release_first.set()
        await asyncio.gather(first, second, screenshot, static)
        assert entered[-2:] == ["/api/jobs/second", "/screenshots/r/e/1.png"]
