...
Modal gateway exposes plus the name each configured model resolves to (see app.models.llm.resolve_modal_model).
...
from app.models.llm import resolve_modal_model  # noqa: E402
from app.models.pricing import CHECKED_ON, DEDICATED_ONLY_MODELS  # noqa: E402
from app.models.registry import CHEAPEST_10, DEFAULT_SWARM, MODELS, cost_rank, discover_modal_models  # noqa: E402
...
def main() -> None:
    ...
    print(
        f"Configured models (Shared Endpoint prices from modal.com/library, checked {CHECKED_ON}; "
        f"cost rank = 0.8*in + 0.2*out):"
    )
    print(
        f"  {'id':22s} {'provider':8s} {'$in/M':>6s} {'$out/M':>6s} {'rank':>5s} vision  model -> sent to the gateway"
    )
    for m in MODELS.values():
        if m.provider == "modal":
            ...
            arrow = m.model
        print(
            f"  {m.id:22s} {m.provider:8s} {m.input_price_per_m:6.2f} {m.output_price_per_m:6.2f} "
    ...
    if DEDICATED_ONLY_MODELS:
        print(
            "Library models without a Shared Endpoint (dedicated GPU billing only, not in the swarm): "
