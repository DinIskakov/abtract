...
Deploy everything with:   modal deploy deploy.py
Dev-serve with:           modal serve deploy.py
...
base_image = _base_build.add_local_python_source("abtract")

# Browser image: Playwright + Chromium for the dom and vision agents.
browser_image = (
    _base_build.run_commands(
        "playwright install-deps chromium",
        "playwright install chromium",
    )
    .add_local_python_source("abtract")
)

# Sandbox image for validating Gemini-written sites (untrusted code never runs in our own containers).
sandbox_image = modal.Image.debian_slim(python_version="3.12").pip_install("beautifulsoup4>=4.12", "lxml>=5.0", "html5lib>=1.1")
