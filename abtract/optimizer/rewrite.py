...
`chat` is imported into this module's namespace so tests can monkeypatch `abtract.optimizer.rewrite.chat`.
"""
...
from abtract.metrics.score import per_model_agent, relative_path
from abtract.modal_app import APP_NAME, DATA_DIR, app, base_image, running_on_modal, sandbox_image, secrets, volumes
from abtract.models.llm import ChatMessage, chat, extract_json  # noqa: F401  (chat is patched in tests)
from abtract.models.registry import get_model
from abtract.optimizer import check_site as check_site_module
from abtract.schemas import Episode, MetricBlock, RunSummary, SiteVersion, Task
...
def _fmt_block(name: str, b: MetricBlock) -> str:
    ...
    return (f"| {name} | {b.episodes} | {100 * b.success_rate:.0f}% | {b.avg_steps:.1f} | {b.avg_duration_s:.1f}s "
...
def metrics_tables(run: RunSummary, episodes: list[Episode]) -> str:
    ...
    lines = [f"# Swarm results for {run.site_id}/{run.site_version} (run {run.run_id})", "", "## Overall", hdr,
             _fmt_block("all", run.overall), "", "## Per model x agent", hdr]
...
def failure_section(run: RunSummary, episodes: list[Episode]) -> tuple[str, set[str]]:
    ...
    for task in run.tasks:
        ...
        lines += ["", f"## Task {task.id} [{task.kind.value}{', trap=' + task.trap if task.trap else ''}] "
                      f"- {len(failed)}/{total} failed",
                  f"Prompt: {task.prompt}", f"Expected: {_expected(task)}"]
        ...
        for e in picked[:MAX_FAILED_EPISODES_PER_TASK]:
            ...
            lines.append(f"- {e.model_id} / {e.agent_kind.value}: failure_mode={e.failure_mode} "
            ...
            for s in e.steps[-TRACE_TAIL_STEPS:]:
                ...
                lines.append(f"    {s.index}. at {_trunc(relative_path(s.url, e.site_id, e.site_version) or '/', 60)} "
...
def propose_rewrite(context: str, *, error_report: str | None = None, previous: dict[str, Any] | None = None,
                    model_id: str = OPTIMIZER_MODEL) -> dict[str, Any]:
...
def apply_rewrite(site_id: str, parent_version: str, proposal: dict[str, Any], *, local: bool = True,
                  context: str | None = None, version: str | None = None) -> SiteVersion:
    ...
    if report and context is not None:
        try:
            ...
            merged = {"notes": fix["notes"] or notes,
