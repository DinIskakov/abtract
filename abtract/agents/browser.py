...
def describe_element(e: dict[str, Any]) -> str:
    ...
    if tag == "button" or role == "button" or (tag == "input" and e.get("type") in ("submit", "button", "image", "reset")):
    ...
    def _block_foreign(self, route: Any, request: Any) -> None:
        try:
            if request.resource_type == "document":
                route.fulfill(status=403, content_type="text/plain", body="abtract: navigation off the site origin is blocked")
            else:
                route.continue_()
    ...
    def select(self, eid: int | None, value: str) -> None:
        ...
        try:
            ...
            try:
                ...
                opts = loc.evaluate("el => Array.from(el.options).map(o => o.text + ' (value=' + o.value + ')').join(' | ')")
