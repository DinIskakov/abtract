...
from abtract.metrics.score import per_model_agent
from abtract.schemas import Episode, MetricBlock, RunSummary
...
def _row(block: MetricBlock) -> list[str]:
    ...
    return [str(block.episodes), _pct(block.success_rate), f"{block.avg_steps:.1f}", f"{block.avg_duration_s:.1f}s",
            f"${block.avg_cost_usd:.4f}", modes]
...
def estimate_table(estimate: dict, budget_usd: float | None = None, *, per_iteration: bool = False) -> Table:
    ...
    title = (f"cost estimate{' per iteration' if per_iteration else ''}: {estimate['episodes']} episodes, "
    ...
    t.add_row("[bold]total", str(estimate["episodes"]), f"[bold {'red' if over else 'green'}]${estimate['total_usd']:.2f}")
    return t
...
def versions_table(results: list[tuple[str, RunSummary]]) -> Table:
    ...
    for version, run in results:
        ...
        t.add_row(version, run.run_id, str(b.episodes), succ, f"{b.avg_steps:.1f}", f"{b.avg_duration_s:.1f}s",
