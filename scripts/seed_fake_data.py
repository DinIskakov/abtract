...
from abtract import store  # noqa: E402
from abtract.models.registry import get_model  # noqa: E402
from abtract.schemas import (  # noqa: E402
    Action,
    AgentKind,
    Episode,
    FailureMode,
...
TASKS: list[Task] = [
    Task(id="task_h100_price", kind=TaskKind.answer, prompt="What does an H100 GPU cost per hour on this platform?",
         expected_answer="$3.95/hr", answer_aliases=["3.95", "$3.95", "3.95/hr"], trap="price_in_image",
         tags=["pricing"]),
    Task(id="task_quickstart", kind=TaskKind.url, prompt="Open the Python SDK quickstart page in the docs.",
         expected_url_pattern=r"docs/quickstart(\.html)?$", trap="hover_menu", tags=["nav"]),
    Task(id="task_waitlist", kind=TaskKind.action, prompt="Join the GPU waitlist with the email agent@example.com.",
         expected_event="waitlist_submit", expected_event_match={"email": "agent@example.com"}, trap="hidden_field",
         tags=["form"]),
    Task(id="task_timeout", kind=TaskKind.answer, prompt="What is the maximum timeout for a single function call?",
         expected_answer="24 hours", answer_aliases=["24h", "86400 seconds", "24 hrs"], trap="accordion",
         tags=["docs"]),
    Task(id="task_changelog", kind=TaskKind.url, prompt="Find the product changelog.",
         expected_url_pattern=r"changelog(\.html)?$", trap="footer_only_link", tags=["nav"]),
    Task(id="task_contact", kind=TaskKind.action, prompt="Use the contact form to ask about enterprise pricing.",
         expected_event="contact_submit", expected_event_match={"topic": "enterprise"}, trap="multi_step_form",
         tags=["form"]),
    Task(id="task_regions", kind=TaskKind.answer, prompt="Which cloud regions are GPUs available in?",
         expected_answer="us-east, us-west, eu-west", answer_aliases=["us-east us-west eu-west"],
         trap="lazy_loaded", tags=["docs"]),
    Task(id="task_status", kind=TaskKind.url, prompt="Go to the system status page.",
         expected_url_pattern=r"status(\.html)?$", trap="modal_overlay", tags=["nav"]),
]

# Which version fixes which trap. Traps not listed are only partially mitigated.
FIXED_IN = {
    "price_in_image": "v1", "hidden_field": "v1", "footer_only_link": "v1", "hover_menu": "v1",
    "accordion": "v2", "multi_step_form": "v2", "lazy_loaded": "v2", "modal_overlay": "v2",
}
...
VERSION_EDITS: dict[str, dict[str, list[tuple[str, str]]]] = {
    "v1": {
        "pricing.html": [('<img src="pricing-table.png" alt="GPU pricing table">',
                          '<table><tr><th>GPU</th><th>Price</th></tr><tr><td>H100</td><td>$3.95/hr</td></tr>'
                          '<tr><td>A100</td><td>$2.10/hr</td></tr></table>')],
        "docs/index.html": [('<span class="menu" data-hover="true">Guides</span>',
                             '<a href="quickstart.html">Python SDK quickstart</a>')],
        "waitlist.html": [('<input name="email" placeholder="you@company.com"><input name="website" class="hp" tabindex="-1">',
                           '<label>Email <input name="email" placeholder="you@company.com"></label>')],
        "index.html": [('<a href="waitlist.html">Waitlist</a> <a href="contact.html">Contact</a></nav>',
                        '<a href="waitlist.html">Waitlist</a> <a href="contact.html">Contact</a> '
                        '<a href="changelog.html">Changelog</a></nav>')],
    },
    "v2": {
        "docs/index.html": [('<details><summary>Limits</summary><p>Maximum function timeout: 24 hours.</p></details>',
        ...
        "index.html": [('<a href="index.html">Home</a>', '<a href="#main" class="skip">Skip to content</a> <a href="index.html">Home</a>')],
    },
}

THOUGHTS = {
    "start": ["The task asks about {topic}; I'll start from the homepage and look for a relevant link.",
              "Landing page loaded. Scanning the navigation for '{topic}'."],
    "nav": ["There is a '{link}' link in the nav, that's probably where the {topic} lives.",
            "Following the '{link}' link.", "The page mentions {topic} further down; navigating to '{link}'."],
    "scroll": ["Nothing relevant above the fold; scrolling to see more.", "Let me scroll down to find the {topic} section."],
    "stuck": ["I can't find any mention of {topic} in the page text. Trying another section.",
    ...
    "give_up": ["I've explored the whole site and cannot find {topic}. Giving up.",
...
TOPIC = {
    "task_h100_price": ("H100 price", "Pricing", "$3.95/hr"), "task_quickstart": ("quickstart", "Docs", "docs/quickstart.html"),
    "task_waitlist": ("waitlist form", "Waitlist", "submitted"), "task_timeout": ("timeout limit", "Docs", "24 hours"),
    "task_changelog": ("changelog", "Changelog", "changelog.html"), "task_contact": ("contact form", "Contact", "submitted"),
    "task_regions": ("GPU regions", "Docs", "us-east, us-west, eu-west"), "task_status": ("status page", "Status", "status.html"),
}
WRONG_ANSWERS = {
...
# --------------------------------------------------------------------------- metrics (kept local: abtract.metrics is built concurrently)
...
def _make_episode(rng: random.Random, run_id: str, site_id: str, version: str, task: Task, model_id: str,
                  agent: AgentKind, started_at: float, success: bool, failure: FailureMode | None) -> Episode:
    ...
    for i in range(n_steps):
        ...
        if last and success:
            if task.kind == TaskKind.action:
                ...
                action = Action(type="answer", text=answer if task.kind == TaskKind.answer else f"Done, I am on {answer}")
            ...
            if failure == "wrong_answer":
                ...
                thought, action = "This looks like the right page.", Action(type="answer", text="I am on the requested page")
            else:
            ...
            err = "TimeoutError: page did not finish loading within 60s" if failure == "timeout" else \
                rng.choice(["Element #7 not found in current observation", "net::ERR_CONNECTION_RESET",
            ...
            action = Action(type="click", id=rng.randint(1, 9)) if agent != AgentKind.text else Action(type="navigate", url=flow[page_i] or "index.html")
        elif rng.random() < 0.5:
            thought, action = rng.choice(THOUGHTS["scroll"]).format(topic=topic), Action(type="scroll", direction="down")
        else:
            thought = rng.choice(THOUGHTS["stuck"]).format(topic=topic)
            action = rng.choice([Action(type="click", id=rng.randint(1, 20)), Action(type="back"),
        ...
        steps.append(Step(index=i, url=url, observation_chars=obs, thought=thought, action=action,
                          latency_ms=latency, error=err, screenshot_path=str(shot) if shot else None))
        usage.add(Usage(input_tokens=obs // 4 + 700, output_tokens=rng.randint(50, 220), llm_calls=1, llm_latency_ms=int(latency * 0.8)))
        t += latency
    ...
    judge = {
        True: {"answer": f"Answer '{answer}' matches expected.", "url": f"Final URL matches /{flow[-1]}$.",
               "action": f"SiteEvent '{task.expected_event}' recorded with matching payload."}[task.kind.value],
        False: {"wrong_answer": "Answer does not match expected value or aliases.",
                ...
                "gave_up": "Agent declared give_up.", "error": "Agent crashed: " + (steps[-1].error or "unknown error"),
                "timeout": "Episode exceeded the timeout."}.get(failure or "error", "Failed."),
    }[success]
    return Episode(
        id=ep_id, run_id=run_id, site_id=site_id, site_version=version, task_id=task.id, agent_kind=agent,
        model_id=model_id, started_at=started_at, finished_at=started_at + t / 1000 + rng.uniform(0.5, 3.0),
        steps=steps, final_answer=steps[-1].action.text if steps and steps[-1].action.type == "answer" else None,
        final_url=final_url, usage=usage, cost_usd=round(usage.cost_usd(spec), 6), success=success,
        judge_reason=judge, failure_mode=None if success else failure,
...
def seed(site_id: str = "demo", src: Path | None = None, *, seed: int = 42, clean: bool = True,
         models: list[str] = MODEL_IDS, agents: list[AgentKind] = AGENT_KINDS, verbose: bool = True) -> dict[str, str]:
    ...
    for vi, version in enumerate(VERSIONS):
        ...
        store.register_version(SiteVersion(site_id=site_id, version=version, parent=prev, created_at=created,
                                           notes=VERSION_NOTES.get(version, ""), changed_files=changed))
        ...
        for task in TASKS:
            for model_id in models:
                for agent in agents:
                    ...
                    ep = _make_episode(rng, run_id, site_id, version, task, model_id, agent, run_started + k * 4.0, ok, fail)
                    ...
                    if task.kind == TaskKind.action and (ok or fail == "wrong_answer"):
                        ...
                        events.append(SiteEvent(name=task.expected_event or "submit", payload=payload, site_id=site_id,
                                                version=version, episode_id=ep.id, ts=ep.finished_at or ep.started_at))
                    k += 1
        for ev in sorted(events, key=lambda e: e.ts):
            store.append_event(ev)
        task_by_id = {t.id: t for t in TASKS}
        run = RunSummary(
            run_id=run_id, site_id=site_id, site_version=version,
            site_url=f"http://localhost:8000/s/{site_id}/{version}/", created_at=run_started,
            finished_at=max(e.finished_at or e.started_at for e in episodes), tasks=TASKS, model_ids=list(models),
            agent_kinds=list(agents), overall=compute_block(episodes),
            per_model=_group(episodes, lambda e: e.model_id), per_agent=_group(episodes, lambda e: e.agent_kind.value),
        ...
        if verbose:
            print(f"{site_id}/{version}: run {run_id}  episodes={run.overall.episodes}  "
