...
from abtract.schemas import SiteVersion  # noqa: E402
...
def import_modal(src: Path, site: str, version: str, parent: str | None, notes: str) -> SiteVersion:
    from abtract.modal_app import data_volume
    ...
    print(f"uploaded {src} -> volume:/sites/{site}/{version} ({len(new.changed_files)} files) in {time.time() - t0:.1f}s")
