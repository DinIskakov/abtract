...
import abtract.agents
from abtract import store
from abtract.config import settings
from abtract.models import llm, registry
from abtract.models.llm import ChatMessage, match_model_name, resolve_modal_model
from abtract.models.pricing import CHECKED_ON, SHARED_ENDPOINT_MODELS
from abtract.models.registry import CHEAPEST_10, DEFAULT_SWARM, MODELS, cost_rank, get_model, select_models
from abtract.schemas import Episode, ModelSpec, Task
from abtract.swarm.runner import BudgetExceeded, estimate_run_cost, run_swarm
...
def test_registry_is_built_from_the_pricing_table():
    ...
    assert DEFAULT_SWARM == CHEAPEST_10[:2]
...
def test_match_model_name_rules():
    names = ["deepseek-ai/DeepSeek-V4.1-Flash", "glm-5-3-flash.us-west.modal.direct", "glm-5-3.us-west.modal.direct",
             "moonshotai/kimi-k3", "Qwen/Qwen3.8-2.4T-A95B"]
    ...
    assert match_model_name("zai-org/GLM-5.3", ["glm-5-3-flash.us-west.modal.direct"]) == \
        "glm-5-3-flash.us-west.modal.direct"                                             # substring, last resort
...
def test_chat_sends_the_resolved_name(monkeypatch, fresh_resolution):
    ...
    class FakeCompletions:
        def create(self, **kw):
            captured.update(kw)
            return SimpleNamespace(id="r1", model=kw["model"],
...
def test_estimate_run_cost_math():
    est = estimate_run_cost(TASKS[:2], ["kimi-k3", "qwen3.8-max"], ["text", "vision"],
                            avg_steps=10, tokens_in_per_step=1000, tokens_out_per_step=100)
...
def test_run_swarm_raises_budget_exceeded_before_launching(tmp_store, monkeypatch):
    ...
    monkeypatch.setattr(abtract.agents, "run_agent", must_not_run)
    with pytest.raises(BudgetExceeded) as ei:
        run_swarm("demo", "v0", TASKS, ["kimi-k3"], ["text"], site_url="u", local=True, run_id="run_budget",
                  budget_usd=0.001)
...
def _one_dollar_agent(agent_kind, model_id, task, site_url, *, run_id, site_id, site_version, screenshot_dir=None):
    return Episode(run_id=run_id, site_id=site_id, site_version=site_version, task_id=task.id, agent_kind=agent_kind,
                   model_id=model_id, final_answer="1", cost_usd=1.0, finished_at=time.time())


def test_local_run_stops_submitting_once_spend_passes_budget(tmp_store, monkeypatch):
    monkeypatch.setattr(abtract.agents, "run_agent", _one_dollar_agent)
    run = run_swarm("demo", "v0", TASKS, ["mock"], ["text", "dom"], site_url="u", local=True, concurrency=1,
                    run_id="run_stop", budget_usd=2.5)  # estimate is $0 (mock), so it launches
