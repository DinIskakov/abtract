...
from abtract.agents import run_agent
from abtract.agents.base import parse_reply
from abtract.models.llm import ChatMessage, LLMError, LLMResponse
from abtract.schemas import AgentKind, Task, TaskKind, Usage
...
def script(monkeypatch, replies):
    ...
    monkeypatch.setattr("abtract.models.llm.chat", fake_chat)
    return calls
...
needs_chromium = pytest.mark.skipif(not chromium_available(), reason="Playwright Chromium not installed (uv run playwright install chromium)")
...
def test_text_form_submit(monkeypatch, site_url):
    ...
    ep = run("text", task("Subscribe jane@example.com to the newsletter on the Pro plan", kind=TaskKind.action), site_url)
...
def test_text_unparseable_reply_is_recorded_and_loop_continues(monkeypatch, site_url):
    calls = script(monkeypatch, ["sure, let me click the pricing link", '{"action": {"type": "hover"}}', A("answer", text="x")])
    ep = run("text", task(), site_url)
...
def test_llm_error_never_raises(monkeypatch, site_url):
    ...
    monkeypatch.setattr("abtract.models.llm.chat", boom)
    ep = run("text", task(), site_url)
