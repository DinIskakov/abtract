"""Tests for the dashboard API (abtract/dashboard/app.py): seed fake data into a tmp store, then hit the routes."""
...
def test_index_and_static(client: TestClient):
    r = client.get("/")
    assert r.status_code == 200 and r.headers["content-type"].startswith("text/html") and "abtract" in r.text
...
def test_static_bundle_fallback(seeded, monkeypatch: pytest.MonkeyPatch):
    ...
    r = c.get("/")
    assert r.status_code == 200 and "abtract" in r.text
...
def test_compare(client: TestClient, seeded):
    ...
    assert cmp["delta"]["success_rate"] == pytest.approx(cmp["b"]["overall"]["success_rate"] - cmp["a"]["overall"]["success_rate"])
