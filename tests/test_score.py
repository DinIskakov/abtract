...
from abtract.metrics.score import (aggregate, answer_matches, compare, judge, metric_block, normalize_answer,
                                   relative_path, url_matches)
from abtract.schemas import Action, Episode, RunSummary, SiteEvent, Step, Task
...
def test_judge_action_reads_site_events(tmp_store):
    task = Task(id="t3", kind="action", prompt="join waitlist", expected_event="waitlist_submit",
                expected_event_match={"email": "Agent@Example.com"})
    ep = _ep(task, failure_mode="max_steps")  # submitted, then wandered until max steps
    store.append_event(SiteEvent(name="waitlist_submit", payload={"email": "agent@example.com", "plan": "pro"},
                                 site_id="demo", version="v0", episode_id="someone_else"))
    store.append_event(SiteEvent(name="waitlist_submit", payload={"email": "agent@example.com", "plan": "pro"},
                                 site_id="demo", version="v0", episode_id=ep.id))
    judge(ep, task)
    ...
    store.append_event(SiteEvent(name="waitlist_submit", payload={"email": "someone@else.com"},
                                 site_id="demo", version="v0", episode_id=wrong_payload.id))
...
def test_aggregate_math(tmp_store):
    ...
    run = RunSummary(site_id="demo", site_version="v0", site_url="http://h/s/demo/v0/", tasks=[t_a, t_b],
                     model_ids=["m1", "m2"], agent_kinds=["text", "dom"])
    now = time.time()
    eps = [
        _ep(t_a, model_id="m1", agent_kind="text", success=True, steps=[_step(0), _step(1)], started_at=now - 10,
            finished_at=now, cost_usd=0.01),
        _ep(t_a, model_id="m2", agent_kind="dom", success=False, failure_mode="wrong_answer", steps=[_step(0)] * 4,
            started_at=now - 20, finished_at=now, cost_usd=0.03),
        _ep(t_b, model_id="m1", agent_kind="dom", success=False, failure_mode="max_steps", steps=[_step(0)] * 6,
            started_at=now - 30, finished_at=now, cost_usd=0.02),
        _ep(t_b, model_id="m2", agent_kind="text", success=True, steps=[], started_at=now - 4, finished_at=now,
            cost_usd=0.0),
    ]
    aggregate(run, eps)
    o = run.overall
