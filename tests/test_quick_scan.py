...
from abtract.agents import base
from abtract.config import settings
from abtract.dashboard.app import JobRequest, _build_job
from abtract.intake.tasks import pick_quick_tasks
from abtract.schemas import Job, RunSummary, Task
...
def test_quick_tasks_skip_inference_and_keep_full_site_tasks(tmp_path, monkeypatch):
    ...
    assert all("timeout_s" not in t or t["timeout_s"] is None for t in json.loads((store.site_dir(sv.site_id, "v0") / "abtract-tasks.json").read_text()))
...
def test_quick_tasks_use_real_homepage_links(tmp_path, monkeypatch):
    ...
    (root / "index.html").write_text('<h1>Build things</h1><a href="pricing.html">Pricing</a>'
...
def test_quick_job_avoids_task_generation_and_final_llm_report(tmp_path, monkeypatch):
    ...
    job = Job(type="intake", url="https://example.com", scan_mode="quick", model_ids=["mock"], agent_kinds=["text", "dom"])
    store.save_job(job)
...
def test_optimizing_quick_scan_reuses_baseline_tasks_and_limits(tmp_path, monkeypatch):
    ...
    run = RunSummary(site_id=sv.site_id, site_version="v0", site_url="https://example.com", scan_mode="quick",
                     tasks=selected, model_ids=["mock"], agent_kinds=["text"], finished_at=time.time())
    store.save_run(run)
