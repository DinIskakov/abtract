...
from abtract.schemas import Action, AgentKind, Episode, RunSummary, Step, Task
from abtract.swarm.runner import build_payloads, run_episode_sync, run_swarm, serve_site_locally
...
def fake_run_agent(agent_kind, model_id, task, site_url, *, run_id, site_id, site_version, screenshot_dir=None):
    ...
    ep = Episode(run_id=run_id, site_id=site_id, site_version=site_version, task_id=task.id, agent_kind=kind,
                 model_id=model_id)
    ep.steps = [Step(index=0, url=site_url, action=Action(type="navigate", url="pricing.html")),
...
def test_run_swarm_local_grid(tmp_store, monkeypatch):
    monkeypatch.setattr(abtract.agents, "run_agent", fake_run_agent)
    run = run_swarm("demo", "v0", TASKS, ["mock", "gemini-flash"], ["text", "dom"],
                    site_url="http://127.0.0.1:1/s/demo/v0/", local=True, concurrency=4, run_id="run_test")
...
def test_agent_exception_becomes_error_episode(tmp_store, monkeypatch):
    ...
    monkeypatch.setattr(abtract.agents, "run_agent", boom)
    payload = build_payloads(RunSummary(site_id="demo", site_version="v0", site_url="u", tasks=TASKS[:1]),
                             TASKS[:1], ["mock"], ["text"])[0]
...
def test_serve_site_locally_static_fallback(tmp_store, monkeypatch):
    monkeypatch.setitem(sys.modules, "abtract.hosting.serve", None)  # force the static fallback
