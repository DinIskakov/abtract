...
from abtract.models.llm import LLMResponse
from abtract.optimizer import rewrite, task_gen
from abtract.optimizer.check_site import check_site
from abtract.schemas import Action, Episode, RunSummary, Step, Task, Usage

INDEX = "<html><body><h1>Modalish</h1><nav><a href='pricing.html'>Pricing</a><a href='docs/'>Docs</a></nav></body></html>"
PRICING = ("<html><body><canvas id='c'></canvas><script>draw('H100 $3.95/hr')</script>"
           "<form action='api/waitlist_submit' method='post'><input name='email'><button>Go</button></form>"
           "<a href='index.html'>Home</a></body></html>")
DOCS = "<html><body><p>docs</p><a href='../index.html'>Home</a><link rel='stylesheet' href='../style.css'></body></html>"
...
def run_with_failures(site):
    task = Task(id="t_price", kind="answer", prompt="How much is an H100 per hour?", expected_answer="3.95",
                answer_aliases=["$3.95"], trap="canvas_pricing")
    run = RunSummary(run_id="run_1", site_id="demo", site_version="v0", site_url="http://h/s/demo/v0/", tasks=[task],
                     model_ids=["mock"], agent_kinds=["text"])
    ep = Episode(run_id="run_1", site_id="demo", site_version="v0", task_id="t_price", agent_kind="text",
                 model_id="mock", success=False, failure_mode="wrong_answer", judge_reason="expected '3.95', got 'unknown'",
                 final_answer="unknown", finished_at=time.time(),
                 steps=[Step(index=0, url="http://h/s/demo/v0/", thought="look for pricing",
                             action=Action(type="navigate", url="pricing.html")),
                        Step(index=1, url="http://h/s/demo/v0/pricing.html", thought="no price text visible",
...
def test_optimize_site_writes_new_version(run_with_failures, monkeypatch):
    ...
    proposal = {"notes": "Added a real table with the H100 price; the text agent could not read the canvas.",
...
def test_broken_link_rejects_version_after_failed_repair(run_with_failures, monkeypatch):
    broken = {"notes": "Linked to a new page.", "files": {"index.html": INDEX.replace("pricing.html", "prices.html")},
              "deleted": []}
...
def test_apply_rewrite_never_deletes_pages_or_escapes_dir(site):
    proposal = {"notes": "n", "files": {"../evil.html": "x", "/abs.html": "y", "notes.txt": "hello"},
...
def test_validate_tasks_filters_uncheckable_tasks(site):
    ...
    raw = [
        {"id": "ok answer", "kind": "answer", "prompt": "H100 price?", "expected_answer": "$3.95/hr",
         "answer_aliases": ["3.95"], "trap": "canvas"},
        ...
        {"kind": "action", "prompt": "join", "expected_event": "api/waitlist_submit",
...
def test_generate_tasks_with_patched_chat(site, monkeypatch):
    reply = {"tasks": [{"id": "t1", "kind": "answer", "prompt": "price?", "expected_answer": "3.95"},
