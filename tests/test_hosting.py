...
from abtract.hosting import serve
...
def test_form_event_without_thanks_page_returns_html(client: TestClient, site: Path):
    ...
    r = client.post("/s/demo/v0/api/contact", data={"msg": "hi"}, headers={"Accept": "text/html"}, follow_redirects=False)
...
def test_cookie_attribution_after_page_view(client: TestClient):
    ...
    r = client.post("/s/demo/v0/api/signup", data={"email": "q"}, headers={"X-Abtract-Episode": "ep_hdr", "Accept": "application/json"})
